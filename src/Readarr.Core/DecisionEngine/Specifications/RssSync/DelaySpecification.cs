...
{
    public class DelaySpecification : IDualDownloadDecisionEngineSpecification
    {
        ...
        public virtual DownloadSpecDecision IsSatisfiedBy(RemoteBook subject, ReleaseDecisionInformation information)
        {
            ...
            if (isPreferredProtocol)
            {
                foreach (var book in subject.Books.Where(b => b.BookFiles.Value.Any()))
                {
                    ...
                    var newQuality = subject.Quality;
            ...
            if (delayProfile.BypassIfHighestQuality)
            {
                ...
                var isBestInProfile = qualityComparer.Compare(subject.Quality.Quality, bestQualityInProfile) >= 0;
            ...
            var bookIds = subject.Books.Select(b => b.Id);
