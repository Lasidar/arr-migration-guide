...
namespace Readarr.Core.IndexerSearch
{
    ...
    public class ReleaseSearchService : ISearchForReleases
    {
        ...
        private List<DownloadDecision> DeDupeDecisions(List<DownloadDecision> decisions)
        {
            ...
            return decisions.GroupBy(d => d.RemoteRelease.Guid)
                .Select(d => d.OrderBy(v => v.Rejections.Count()).ThenBy(v => v.RemoteRelease?.IndexerPriority ?? IndexerDefinition.DefaultPriority).First())
                .ToList();
        }
    }
}
